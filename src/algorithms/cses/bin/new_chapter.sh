...
Generate a new cses chapter.

Available options:

-h, --help      Print this help and exit
First argument: The title of the chapter 
...
EOF
  ...
  echo "$(( index + 1 )). [[${corrected_files[$index]}]]" >> $index_file

  if (( index > 0 )); then
    prev_index="$((index - 1))"
    prev_item="${corrected_files[$prev_index]}"
    echo "Prev: [[$prev_item]]" >> $FILE_NAME
  fi

  if (( index < len - 1 )); then
    next_index="$((index + 1))"
    next_item="${corrected_files[$next_index]}"
    echo "Next: [[$next_item]]" >> $FILE_NAME
  fi
