# How do you make a system call faster?

To make a system call faster, you can add it to the vdso (virtual dynamic shared object), which implements a set of system calls by mapping them into every processes' memory. On x86, they are `clock_gettime()`, `gettimeofday()`, `time()`, and `getcpu()`. On my computer, it's about 4x faster to use the vdso version than the real system call (90 microseconds vs 22 microseconds). On x86, it is the size of a page (4KB). There's some room to add your own system calls. This article: [Adding Syscalls to vdso](https://lwn.net/Articles/615809/) has more detail.

## Code

Copy the code, compile it, and run it `./a.out $FILE_NAME` and `objdump -d $FILE_NAME` to see the dissasembled vdso.
