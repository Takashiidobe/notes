...
**3.1** Prove, using induction but also without it, that $n(n + 1)$ is an even number for every non-negative
  integer $n$.
  ...
  **3.3** Observe that the number $n(n + 1)/2$ is the number of handshakes among $n + 1$ people. Suppose that
  everyone counts only handshakes with people older than him/her (pretty snobbish, isn’t it?). Who will count the
  largest number of handshakes? How many people count 6 handshakes?
  ...
  **3.5** Prove the following identity:
  $1 \cdot 2 + 2 \cdot 3 + 3 \cdot 4 + \dots + (n - 1) \cdot n = \dfrac{(n - 1) \cdot n \cdot (n + 1)}{3}$
...
Prev: [[02-let-us-count]]
Next: [[04-counting-subsets]]
