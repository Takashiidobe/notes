    ...
    data.insert(
        ...
        RispExp::Func(|args: &[RispExp]| -> Result<RispExp, RispErr> {
            ...
            let first = *floats
                .first()
                .ok_or(RispErr::Reason("expected at least one number".to_string()))?;
            let rest = floats[1..].iter().fold(0.0, |sum, a| sum + a);

            Ok(RispExp::Number(first - rest))
        }),
    );
    data.insert(
        "*".to_string(),
        RispExp::Func(|args: &[RispExp]| -> Result<RispExp, RispErr> {
            let total = parse_list_of_floats(args)?
                .iter()
                .fold(1.0, |total, a| total * a);
    ...
    data.insert(
        ...
        RispExp::Func(|args: &[RispExp]| -> Result<RispExp, RispErr> {
            ...
            let first = *floats
                .first()
                .ok_or(RispErr::Reason("expected at least one number".to_string()))?;
            let rest = floats[1..].iter().fold(1.0, |div, a| div * a);
            Ok(RispExp::Number(first / rest))
        }),
    );
    data.insert(
